...
The extensions were designed and added by the named authors under the supervision of Dr Tomasz Kurowski between March and May 2025. 
...
The genome browser relies on \texttt{Jbrowse} [TODO: Reference] extensively. Since Jbrowse is written in \texttt{React} [TODO: Reference], it is embedded in the new angular app via the \texttt{ReactComponentDirective} provided by the \texttt{@ngeenx/ngx-react} package on NPM. 
...
\begin{itemize}
    ...
    \item (4) be able to generate DNA barcodes for the various cultivars and species in the dataset; and,  
...
\begin{itemize}
    ...
    \item npm registry \url{https://registry.npmjs.org/}
...
\begin{enumerate}
    ...
    \item Change to the root of the Tersect-browser repo and run:\\
        \verb+nvm use 16+\\
        \verb+npm start+
    \item Change to the Tersect-browser/extension/genome-browser/ directory and run:\\
        \verb+nvm use 22+\\
        \verb+npm start+
...
\textit{Tersect Browser+} comprises Tersect CLI, Tersect Browser and four extensions that meet the \textit{Tersect Browser+} use case in Section 3. These were: 
...
On reviewing node-compatible browser tools such as CRAMER, JBrowse was selected as the preferred solution, due to its high customisability, up-to-date features and active technical support. The initial build of JBrowse into \textit{TersectBrowser+}, focused on retaining features most relevant to the user aims. This included bringing the styling of the panel in line with the rest of the browser page. The extension also involved generating a popup window of the same JBrowse component to be initialised when the user identified a specific accession or bin region of interest. 
...
\subsubsection{Jbrowse TersectBrowser Syncing}
...
The preparation process is automated with the \verb*|{build_annotate_per_chrom.sh}| script (Supplementary File 1). Its example usage is; 

\begin{verbatim}
    build_annotate_per_chrom.sh tracks.json index.tsi genes.gff3.gz [outdir]
\end{verbatim}

Where \texttt{tracks.json} is the JBrowse tracks configuration file (TODO: Reference), \texttt{index.tsi} is the Tersect index file, \texttt{genes.gff3.gz} is the compressed GFF3 gene annotation file, and \texttt{[outdir]} is the output directory for the generated per-chromosome VCF and annotation files.
...
The Trix file generation is automated with the \verb*|(tersect_trix_builder.sh)| script (Supplementary File 2).


\subsubsection{Handling A Search Request}

An API endpoint was added to the \texttt{tersect-router} file on the backend to support search functionality. The structure of the endpoint is as follows:
...
\subsubsection{Searching by Interval}
...
\subsubsection{Searching by Gene Name}
...
Variant search can also be triggered from:
\begin{itemize}
  \item Each bin on the bin plot.
  \item A selected range on the scale plot.
\end{itemize}

These triggers emit a custom event \texttt{tersect-search-variants}, which is being listened for on the react component. This sets the interval in the search dialog and opens the modal for user interaction.
