...
specified location within the repository: \textit{Tersect-browser/\(\sim \)/mongo-data/gp\_data\_copy/}
\begin{itemize}
    ...
    \item Download (or generate) an annotation GFF file for the resequenced dataset using SNPeff.  Index the GFF file. Run tabix on this file to create a .tbi index file.
\end{itemize}
\begin{itemize}
...
\subsubsection{Rationale}
A key aim for plant breeders when comparing their own plant strains with other cultivars in resequenced datasets is to be able to identify their strain at the genome level. This would allow for protection of intellectual property in the case of an advantageous new introgression that can be uniquely identified. The Barcoding extension allows this unique identification by generating a range of barcodes with a user-specified maximum number of SNPs, so that primers for this barcode are easily generated in a wetlab environment. Metrics comparing these putative barcodes are also provided to the user, and available for local download. 
